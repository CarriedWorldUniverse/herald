...
- **Cert strategy: per-host, on-demand — NOT a wildcard.** A broad `*.<tier>.carriedworld.com` wildcard is deliberately avoided: in a multi-tenant context it has a large blast radius (one key impersonates every tenant) and enables subdomain "camping" (any name under the wildcard is *implicitly* certed, so a dangling/misconfigured DNS record is automatically TLS-valid without passing an issuance challenge). Instead, each tenant hostname gets its **own** cert issued only when the tenant is provisioned + DNS-validated:
  - **Edge:** Cloudflare **Total TLS** (auto-issues an individual edge cert per hostname that has a DNS record; same ~$10/mo ACM subscription, but per-host). A removed tenant's DNS record removal takes its cert with it.
  - **Bring-your-own domains** (`git.acme.com`): Cloudflare for SaaS custom hostnames (per-host; first 100 free).
  - **Origin (k3s):** `cert-manager` issues a per-Ingress-host cert via ACME (Let's-Encrypt), not a wildcard.
  - **Hygiene (root-cause fix):** tenant deprovisioning MUST delete the DNS record — no dangling CNAMEs. Per-host issuance stops rewarding the mistake, but removing the record is the actual fix for takeover risk.
...
- **Tier certs** — decided: **per-host on-demand, not wildcard** (Total TLS at the edge / cert-manager per-Ingress-host at origin / Cloudflare-for-SaaS for BYO domains) to cut blast radius + kill subdomain-camping. Plus the deprovision-deletes-DNS hygiene rule. Same ~$120/yr ACM subscription as a wildcard would have cost, but per-host. Confirm exact edge config (Total TLS vs per-host Cloudflare-for-SaaS) at edge stand-up.
