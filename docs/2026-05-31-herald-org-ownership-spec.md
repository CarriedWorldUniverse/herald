...
- **Cert strategy: per-host, short-term rolling, NOT a wildcard.** A broad `*.<tier>.carriedworld.com` wildcard is deliberately avoided: in a multi-tenant context it has a large blast radius (one key impersonates every tenant) and enables subdomain "camping" (any name under the wildcard is *implicitly* certed, so a dangling/misconfigured DNS record is automatically TLS-valid without passing an issuance challenge). Each tenant hostname gets its **own** cert, issued only when the tenant is provisioned + DNS-validated, and **rolled on a short cycle** (Let's-Encrypt; the ~6-day short-lived profile preferred, so a leaked cert self-expires fast with minimal reliance on revocation). Two-layer termination (decided 2026-05-31):
  - **Public edge — Cloudflare (proxied):** Cloudflare terminates public TLS with its own auto-rolled managed cert (LE-backed), keeping DDoS / WAF / origin-hiding. **Total TLS** issues an individual per-host edge cert for each hostname that has a DNS record (no wildcard); a removed tenant's DNS-record deletion takes its cert with it.
  - **CF→origin (Full-strict) — our LE rolling certs:** `cert-manager` on k3s issues per-Ingress-host certs via the **Let's-Encrypt ACME issuer using DNS-01** (Cloudflare API token) — short-lived, auto-renewed, and **DNS-01 works behind NAT** so it runs on dMon today and on AWS later unchanged. This is the cert lifecycle we own.
  - **Bring-your-own domains** (`git.acme.com`): Cloudflare for SaaS custom hostnames (per-host; first 100 free).
  - **Hygiene (root-cause fix):** tenant deprovisioning MUST delete the DNS record — no dangling CNAMEs. Per-host + short-lived issuance stops rewarding the mistake, but removing the record is the actual fix for takeover risk.
...
- **Tier certs** — decided: **per-host, short-term rolling, not wildcard.** Cloudflare proxied edge terminates public TLS (Total TLS, per-host, auto-rolled LE-backed managed certs; keeps DDoS/WAF/origin-hiding); **cert-manager + Let's-Encrypt (DNS-01 via Cloudflare token)** issues the short-lived rolling certs on the CF→origin Full-strict hop, which we control. Cloudflare-for-SaaS for BYO domains. Deprovision-deletes-DNS hygiene rule. Same ~$120/yr ACM subscription. Confirm the LE profile (90-day vs ~6-day short-lived) + cert-manager issuer config at edge stand-up.
